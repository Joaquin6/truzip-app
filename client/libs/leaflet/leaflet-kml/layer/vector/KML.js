...
L.Util.extend(L.KML, {
	...
	parseGroundOverlay : function(overlay, style, options) {
		var i, j, el;
		var blobURLs = [];
		//keeps track of blob URLs created by KMZ processor for contained image files, these need to be release when the object is removed from the map
		...
		if (blobURLs.length > 0) {//Setup a listener to remove the blobs from memory by their blob:/ URLs when this feature group is removed from the map
			...
			layer.on('layerremove', function() {
				...
				// Remove the blob containing the KML document
				...
				if (key) {
					...
					if (key === 'color') {
						...
						if (value.substr(0, 5).toLowerCase() == 'blob:') {
							blobURLs.push(value);
							//keep track of this blob URL to release the memory when this layer is removed from the map
							options.href = value;
